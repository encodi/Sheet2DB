"""
SQL Server adapter for DataMigrator

Implements SQL Server database connectivity using SQLAlchemy and pyodbc.
"""
...
class MSSQLAdapter(BaseAdapter):
    """
    SQL Server database adapter using SQLAlchemy and pyodbc.
    ...
    def __init__(self, connection_string: str, db_name: str, options: Dict[str, Any] = None):
        ...
        Args:
            connection_string: SQL Server connection string (mssql+pyodbc://user:pass@host:port)
        ...
        self.schema_name = options.get('schema_name', 'dbo')
        ...
        self.full_connection_string = f"{base_connection}/{db_name}?driver={self.driver.replace(' ', '+')}"
        self.server_connection_string = f"{base_connection}/master?driver={self.driver.replace(' ', '+')}"
    ...
    def _get_server_connection_info(self) -> Dict[str, str]:
        ...
        if not match:
            raise ValueError(f"Invalid SQL Server connection string format")
    ...
    def create_database_if_not_exists(self) -> None:
        ...
        Uses pyodbc to connect to the server and create the database.
        """
    ...
    def _map_sql_type_to_sqlalchemy(self, column_schema: ColumnSchema):
        """Map generic SQL types to SQLAlchemy SQL Server types."""
        ...
        if sql_type == 'INTEGER':
            ...
            # SQL Server has different limits for VARCHAR
            if length > 8000:
                return Text
    ...
    def insert_dataframe(self, table_name: str, df: pd.DataFrame, batch_size: int = 1000) -> int:
        ...
        try:
            # Use pandas to_sql for SQL Server
            df_copy.to_sql(
